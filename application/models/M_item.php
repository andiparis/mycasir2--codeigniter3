...
class M_item extends CI_Model {
	 ...
	 // start datatables
    var $column_order = array(null, 'barcode', 'p_item.name', 'category_name', 'unit_name', 'price', 'discount', 'stock'); //set column field database for datatable orderable
	...
	public function add($post) {
		$total_discount = (($post['discount']/100) * $post['price']);
		$params = [
			'barcode' => $post['barcode'],
			'name' => $post['product_name'],
			'category_id' => $post['category'],
			'unit_id' => $post['unit'],
			'price' => $post['price'] - $total_discount,
			'discount' => $post['discount'],
			'before_discount' => (($post['discount']/100) * $post['price']),
			'image' => $post['image']
		];
